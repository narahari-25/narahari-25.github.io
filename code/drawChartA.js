function drawChartA(file1Data, file2Data, file3Data, file4Data) {
    ...
    profitA4 = profitA4 / 5;
    const data = [
        { letter: 'KRBL', expenditure: totalExpenditureA1, profit: profitA1 },
        { letter: 'LT', expenditure: totalExpenditureA2, profit: profitA2 },
        { letter: 'HM', expenditure: totalExpenditureA3, profit: profitA3 },
        { letter: 'ADF', expenditure: totalExpenditureA4, profit: profitA4 }
    ];
    ...
    function barClickHandler(d) {
        ...
        loadData().then(([file1Data, file2Data, file3Data, file4Data]) => {
            if (d.letter == 'KRBL') {
                ...
                drawChartA2(file2Data);
            }
            else if (d.letter == 'HM') {
                drawChartA2(file3Data);
            }
            else if (d.letter == 'ADF') {
                drawChartA2(file4Data);
            }
    ...
    svg.selectAll(".profit")
        ...
        .on("mouseover", function (event, d) {
            if (d.letter == 'KRBL' && countA == 1) {
                showTooltip(event, `Loss: ${d.profit}`);
            }
            else if (d.letter == 'LT' && countB == 1) {

                showTooltip(event, `Loss: ${d.profit}`);
            }
            else if (d.letter == 'HM' && countC == 1) {

                showTooltip(event, `Loss: ${d.profit}`);
            }
            else if (d.letter == 'ADF' && countD == 1) {
