function drawChartB(file1Data, file2Data, file3Data, file4Data) {
    d3.select("#chart").selectAll("svg").remove();
    d3.selectAll(".tooltip").remove();
    d3.select("#chart").selectAll("g").remove();

    const margin = { top: 50, right: 0, bottom: 200, left: 400 };
    ...
    profitA4 = profitA4 / 5;
    const data = [
        { letter: 'Hester-Bio', expenditure: totalExpenditureA1, profit: profitA1 },
        { letter: 'Ind-Swift', expenditure: totalExpenditureA2, profit: profitA2 },
        { letter: 'Kopran', expenditure: totalExpenditureA3, profit: profitA3 },
        { letter: 'Themis-Medicare', expenditure: totalExpenditureA4, profit: profitA4 }
    ];
    ...
    function barClickHandler(d) {
        ...
        loadData().then(([file1Data, file2Data, file3Data, file4Data, ph1Data, ph2Data, ph3Data, ph4Data]) => {
            if (d.letter == 'Hester-Bio') {
                drawChartA2(ph1Data);
            }
            else if (d.letter == 'Ind-Swift') {
                drawChartA2(ph2Data);
            }
            else if (d.letter == 'Kopran') {
                drawChartA2(ph3Data);
            }
            else if (d.letter == 'Themis-Medicare') {
                drawChartA2(ph4Data);
            }
    ...
    svg.selectAll(".profit")
        ...
        .on("mouseover", function (event, d) {
            if (d.letter == 'Hester-Bio' && countA == 1) {
                showTooltip(event, `Loss: ${d.profit}`);
            }
            else if (d.letter == 'Ind-Swift' && countB == 1) {

                showTooltip(event, `Loss: ${d.profit}`);
            }
            else if (d.letter == 'Kopran' && countC == 1) {

                showTooltip(event, `Loss: ${d.profit}`);
            }
            else if (d.letter == 'Themis-Medicare' && countD == 1) {
